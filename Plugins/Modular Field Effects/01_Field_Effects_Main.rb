...
module Fields
  ...
  for move in moves
    ...
    water.push(GameData::Move.get(mv).id) if (mv.type == :WATER && mv.category != 2 && mv.base_damage >= 65 && ![:HYDROSTEAM,:SCALD,:STEAMERUPTION].include?(mv.id))
    ...
    swamp.push(GameData::Move.get(mv).id) if (mv.type == :ROCK && mv.base_damage >= 75)
  ...
  QUAKE_GRASSY = [:EARTHQUAKE,:BULLDOZE,:MAGNITUDE]
  ...
  MELT_MOVES = IGNITE_MOVES + [:SCALD,:STEAMERUPTION,:HYDROSTEAM]
  RICOCHET_MOVES = [:DAZZLINGGLEAM,:MIRRORSHOT,:MIRRORRUSH,:LIGHTOFRUIN,:MAKEITRAIN,:FLASH,:FLASHCANNON,:AURORABEAM,:MOONBLAST,:STARBEAM,
    :PHOTONGEYSER,:ICEBEAM,:LUSTERPURGE,:MISTBALL,:SNIPESHOT,:BUBBLEBEAM,:PSYBEAM,:AURASPHERE,:CHARGEBEAM,:THUNDERBOLT,:FLAMEBURST,:SEARINGSHOT]
  LIGHT_MOVES = [:DAZZLINGGLEAM,:FLASH,:PHOTONGEYSER,:LUSTERPURGE,:AURORABEAM,:DISCHARGE]
  FAIRY_LIGHTS_MOVES = [:PHOTONGEYSER,:LUSTERPURGE,:AURORABEAM,:DISCHARGE,:MISTBALL,:FLASH,:STARBEAM]
  ...
  IGNITE_WEATHER = [:Rain,:HeavyRain,:Storm,:AcidRain]
  DOUSE_WEATHER = [:Sun,:HarshSun]
  MELT_WEATHER = [:Hail,:Sleet]
  LIGHT_WEATHER = [:Eclipse]

  def self.ignite?
    return !IGNITE_WEATHER.include?($field_weather)
  end

  def self.douse?
    return !DOUSE_WEATHER.include?($field_weather)
  end

  def self.light?
    return !LIGHT_WEATHER.include?($field_weather)
  end

  def self.melt?
    return !MELT_WEATHER.include?($field_weather)
  end
...
class PokemonTemp
  ...
  def pbPrepareBattle(battle)
    ...
    if battleRules["defaultField"].nil?
      ...
      pbSetField(battle.defaultField)
    end
...
class PokeBattle_Battle
  ...
  def pbStartBattleCore
    ...
    if fe && @field.field_effects == :Castle
      ...
      when "dream"
        @field.effects[PBEffects::TrickRoom] = 999
        pbDisplay(_INTL("The Dream field makes slow Pokémon move first!"))
      end
    end
    # Abilities upon entering battle
    pbOnActiveAll
    $field_weather = pbWeather
    # Main battle loop
    pbBattleLoop
  end
  ...
  def pbEndOfBattle
    ...
    item_ban = []
    GameData::Item.each {|berry| item_ban.push(berry.id) if berry.is_berry? }
    GameData::Item.each {|gem1| item_ban.push(gem1.id) if gem1.is_gem? }
    item_ban.push(:BERRYJUICE)
    item_ban.push(:THROATSPRAY)
    pbParty(0).each_with_index do |pkmn,i|
      next if !pkmn
      ...
      if @opponent
        pkmn.item = $olditems[i] unless pkmn.item == nil && item_ban.include?($olditems[i])
      else
  ...
  def pbEORField(battler)
    ...
    if fe
      ...
      when :Electric,:Magnetic
        if battler.canHeal? && battler.hasActiveAbility?(:VOLTABSORB)
      ...
      when :Swamp, :Poison
        if battler.affectedByTerrain?
          if battler.hasActiveAbility?(:POISONHEAL) && battler.canHeal?
            PBDebug.log("[Lingering effect] Swamp Field heals #{battler.pbThis(true)}")
            battler.pbRecoverHP(battler.totalhp / 16)
            pbDisplay(_INTL("{1}'s HP was restored.", battler.pbThis))
          else
            if @field.field_effects == :Poison
              PBDebug.log("[Lingering effect] Poison Field poisons #{battler.pbThis(true)}")
              battler.pbPoison if battler.pbCanPoison?(nil,false)
              pbDisplay(_INTL("{1}'s was poisoned.", battler.pbThis))
            end
          end
        end
      when :Lava
...
def pbSetField(terrain,sprites)
  BattleScripts.pbSetFieldEffect(terrain,sprites)
end

#Field Changes due to Move Usage
class PokeBattle_Scene
  def pbChangeField(terrain)
    pbSetField(terrain,@sprites)
  end
end
...
#Move Accuracy Changes for Field Effects
    ...
    if fe
      if fe[:type_type_change].keys != nil
        for type_mod in fe[:type_type_change].keys
          if ret == type_mod
            ret = GameData::Type.get(fe[:type_type_change][type_mod]).id
        ...
        for key in fe[:type_type_mod].keys
          if fe[:type_type_mod][key] == moveType
            ...
            for mess in fe[:type_mod_message].keys
              $type_mod_message1 = mess if fe[:type_mod_message][mess] == moveType
            end
          end
        end
      end
      $type_mod_message2 = nil
      $type_mod_message3 = nil
            ...
            if fe[:move_type_mod][mv].any? {|md| md.include?(self.id)}
              ...
              for msg in fe[:move_messages].keys
                for mo in fe[:move_damage_boost].keys
                 type_mod_message2 = msg if fe[:move_messages][msg].any? {|m| m.include?(self.id)} && !fe[:move_damage_boost][mo].any? {|mm| mm.include?(self.id)}
                end
              end
            end
          else
            if fe[:move_type_mod][mv] == self.id
              eff = Effectiveness.calculate_one(mv,defType)
              ret *= eff.to_f / Effectiveness::NORMAL_EFFECTIVE_ONE
              for msg in fe[:move_messages].keys
                for mo in fe[:move_damage_boost].keys
                 type_mod_message3 = msg if fe[:move_messages][msg] == self.id
                end
              end
            end
          end
  ...
  def pbCalcDamageMultipliers(user,target,numTargets,type,baseDmg,multipliers)
    ...
    if !ignoresReflect? && !target.damageState.critical &&
       !user.hasActiveAbility?([:INFILTRATOR,:JESTERSTRICK])
    ...
    fe = FIELD_EFFECTS[@battle.field.field_effects]
    if fe
      prev_field = @battle.field.field_effects
       if fe[:field_changers] != nil
         priority = @battle.pbPriority(true)
         msg = nil
         for fc in fe[:field_changers].keys
          if @battle.field.field_effects != :None
            next unless fe[:field_changers][fc].include?(self.id)
            if fe[:field_change_conditions][fc] != nil && fe[:field_change_conditions][fc] == true
              ...
              fe = FIELD_EFFECTS[@battle.field.field_effects]
              @battle.scene.pbChangeField(fc)
              @battle.scene.pbRefreshEverything
              priority.each do |pkmn|
                if prev_field == :Magnetic
                  if pkmn.effects[PBEffects::MagnetRise] != 0
                    pkmn.effects[PBEffects::MagnetRise] = 0
                    @battle.pbDisplay(_INTL("{1}'s electromagnetism wore off!",pkmn.name))
                  end
                end
     ...
     #Field Effect Type Boosts
         ...
         for key in fe[:type_damage_change].keys
           if @battle.field.field_effects != :None
            ...
            if mesg == true
              ...
              mesg = false if $test_trigger == false
            end
           end
         end
       end
       ...
       if $test_trigger == false
         if $type_mod_message1
           @battle.pbDisplay(_INTL("{1}",$type_mod_message1))
         end
         if $type_mod_message2
           @battle.pbDisplay(_INTL("{1}",$type_mod_message2))
         end
         if $type_mod_message3
           @battle.pbDisplay(_INTL("{1}",$type_mod_message3))
         end
       end
      #Field Effect Defensive Modifiers
       if fe[:defensive_modifiers] != nil
        ...
        for d in fe[:defensive_modifiers].keys
          next unless target.pbHasType?(fe[:defensive_modifiers][d][0])
          if fe[:defensive_modifiers][d][1] == "fullhp"
            multipliers[:final_damage_multiplier] /= d if target.hp == target.totalhp
          elsif fe[:defensive_modifiers][d][1] == "physical"
            multipliers[:defense_multiplier] *= d if physicalMove?
          elsif fe[:defensive_modifiers][d][1] == "special"
            multipliers[:defense_multiplier] *= d if specialMove?
       ...
       if fe[:side_effects] != nil
        ...
        for eff in fe[:side_effects].keys
          if (fe[:side_effects][eff].is_a?(Array) && fe[:side_effects][eff].include?(self.id)) || (!fe[:side_effects][eff].is_a?(Array) && type == GameData::Type.get(fe[:side_effects][eff]).id)
            case eff
            when "sand"
              @battle.pbStartWeather(nil,:Sandstorm,5)
            when "cinders"
