...
class PokemonPauseMenu
  ...
  def pbStartPokemonMenu
    ...
    commands[cmdDexnav = commands.length]  = _INTL("DexNav") if $game_switches[401] && !$game_switches[LvlCap::Expert]
    commands[cmdPC = commands.length]  = _INTL("PC Box Link") if $Trainer.party_count > 0 && $game_switches[LvlCap::Ironmon] == false && $game_switches[Settings::GAUNTLET_SWITCH] == false
    ...
    loop do
      ...
      if cmdPokedex>=0 && command==cmdPokedex
        ...
        if $game_switches[LvlCap::Expert]
          pbMessage("Which would you like to do?\\ch[34,6,Access PC, Export...,Level All...,Return Items...,Cancel]")
          ...
          when 1
            export_all
          when 2
            pbMessage(_INTL("This will level all able Pokémon to the level cap in both your party and your box."))
            pbMessage(_INTL("This will not level the ones in the last 2 boxes."))
            if pbConfirmMessage(_INTL("Are you sure you want to level all?"))
              level_cap_all
              pbMessage(_INTL("All Pokémon raised to level cap!"))
            end
          when 3
            pbMessage(_INTL("This will return all items held by your Pokémon in both your party and your box."))
          ...
          pbMessage("Which would you like to do?\\ch[34,5,Access PC,Heal: #{pbGet(74)},Export...,Level All...,Return Items...,Cancel]")
          ...
          elsif $game_variables[34] == 2
            export_all
          elsif $game_variables[34] == 3
            pbMessage(_INTL("This will level all able Pokémon to the level cap in both your party and your box."))
            pbMessage(_INTL("This will not level the ones in the last 2 boxes."))
            if pbConfirmMessage(_INTL("Are you sure you want to level all?"))
              level_cap_all
            end
          elsif $game_variables[34] == 4
            pbMessage(_INTL("This will return all items held by your Pokémon in both your party and your box."))
            if pbConfirmMessage(_INTL("Do you wish to continue?"))
              return_items
            end
...
class PokemonSummary_Scene
  ...
  def change_Level
    ...
    elsif pkmn.level > LvlCap.cap
      pbMessage(_INTL("This Pokémon is over the level cap already."))      
    else
      pkmn.level = LvlCap.cap
      pkmn.calc_stats
      dorefresh = true
      if dorefresh
        drawPage(@page)
  ...
  def pbHowToEvolve
    pkmn = GameData::Species.get_species_form(@pokemon.species,@pokemon.form)
    wartime = @pokemon.shiny_locked? ? "Wartime " : ""
    evolution = GameData::Species.get(pkmn).get_evolutions
    map_data = pbLoadMapInfos
    if evolution.length == 1
      nextevo = GameData::Species.get(evolution[0][0]).name
      evo_name = GameData::Evolution.get(evolution[0][1]).real_name
      case evo_name
        when "Item"
          param = " with #{GameData::Item.get(evolution[0][2]).name}"
        when "HoldItem"
          param = " holding #{GameData::Item.get(evolution[0][2]).name}"
        when "HasMove"
          param =  " while knowing #{GameData::Move.get(evolution[0][2]).name}"
        when "Location"
          param = " at #{map_data[evolution[0][2]].name}"
        else
          param = " #{evo_name} #{evolution[0][2]}"
        end
      pbMessage(_INTL("Evolves into: {1}{2}{3}",wartime,nextevo,param))
    elsif evolution == []
      pbMessage(_INTL("This Pokémon does not evolve."))
    else
      evolution.each do |evo|
        nextevo = GameData::Species.get(evo[0]).name
        next if $game_switches[LvlCap::Expert] && nextevo == "Shedinja"
        evo_name = GameData::Evolution.get(evo[1]).real_name
        if evo_name == "Item" && evo[2] == :ALTERNATESTONE
          form = "-Alola" if @pokemon.alolan?
          form = "-Galar" if @pokemon.galarian?
          form = "-Hisui" if @pokemon.hisuian?
        elsif evo_name == "Item" && evo[2] == :BLOODMOONSTONE
          form = "-Blood Moon" if @pokemon.paldean?
        else
          form = ""
        end
        evo_name = GameData::Evolution.get(evo[1]).real_name
        case evo_name
        when "Item"
          param = " with #{GameData::Item.get(evo[2]).name}"
        when "HoldItem"
          param = " holding #{GameData::Item.get(evo[2]).name}"
        when "HasMove"
          param =  " while knowing #{GameData::Move.get(evo[2]).name}"
        when "Ninjask"
          param = " at level #{evo[2]}"
        when "Location"
          param = " at #{map_data[evo[2]].name}"
        else
          param = "#{evo_name} #{evo[2]}"
        end
        pbMessage(_INTL("Evolves into #{wartime}#{nextevo}#{form}:\n{1}",param))
      end
    end
  end
  ...
  def pbOptions
    ...
    cmdEvolution = -1
    cmdMark     = -1
    evos = GameData::Species.get_species_form(@pokemon.species,@pokemon.form).get_evolutions
    if !@pokemon.egg?
      commands[cmdGiveItem = commands.length] = _INTL("Give item")
      commands[cmdTakeItem = commands.length] = _INTL("Take item") if @pokemon.hasItem?
      commands[cmdPokedex = commands.length]  = _INTL("View Pokédex") if $Trainer.has_pokedex
      if ($game_switches[75] || $game_switches[LvlCap::Expert])
        commands[cmdMinGrind = commands.length] = _INTL("Minimal Grinding Options...") if @page == 2 || @page == 3 || @page == 4
      end
      commands[cmdEvolution = commands.length]  = _INTL("How to Evolve...") if [2,3,4].include?(@page) && evos != []
    end
    ...
    if cmdGiveItem>=0 && command==cmdGiveItem
      ...
      cmdStatChange = -1
      cmdAbility = -1
      min_grind_commands[cmdLevel = min_grind_commands.length] = _INTL("Level to Cap") if (@page == 2 || @page == 3 || @page == 4)
      if !$game_switches[LvlCap::Expert]
    ...
    loop do
      ...
      GameData::Species.get_species_form(pkmn.species,pkmn.form).get_evolutions(true).each do |evo|   # [new_species, method, parameter, boolean]
          if evo[1].to_s.start_with?('Item')
            can_evo = true if $PokemonBag.pbHasItem?(evo[2]) && pkmn.check_evolution_on_use_item(evo[2])
      ...
      if !pkmn.egg?
        if pkmn.mail
          ...
          commands[cmdEvolve = commands.length]   = _INTL("!! Evolve !!") if evos != [] && can_evo
      ...
      if cmdSummary>=0 && command==cmdSummary
        ...
        if evoreqs[newspecies] # requires an item
          ...
          pkmn.form = 1 if [:ALTERNATESTONE,:BLOODMOONSTONE].include?(GameData::Item.get(evoreqs[newspecies]).id)
...
class PokemonPokedexInfo_Scene
  ...
  def drawPageArea
    ...
    textpos.push([_INTL("Level"), 26, 46, 0, base, shadow])
    textpos.push([_INTL("Move"), 172, 46, 1, base, shadow])
    idx = 1
    xdx = 0
    for move_data in moves
      ...
      idx += 1
      if idx > 18
        textpos.push([_INTL("Level"), 266, 46, 0, base, shadow])
        textpos.push([_INTL("Move"), 412, 46, 1, base, shadow])
        xdx += 1
        idx = 1
      end
    end
