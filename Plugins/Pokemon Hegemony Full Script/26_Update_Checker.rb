...
def rewrite_version
  File.open("version", "wb") { |f|
    version = Downloader.toString(Hegemony::VERSION_LINK)
    f.write(version)
  }
end
...
def read_changes
  ...
  path = "Data/Changes/#{version}/changes"
  files = []
...
def read_deleted
  version = Settings::GAME_VERSION
  path = "Data/Changes/#{version}/deleted"
  files = []
  Compiler.pbCompilerEachPreppedLine(path) { |line, line_no|
    Graphics.update if line_no % 200 == 0
    files.push(line)
...
module Hegemony
  VERSION_LINK = $BETA ? "https://raw.githubusercontent.com/phantombass/Pokemon-Hegemony/master/version.txt" : "https://raw.githubusercontent.com/phantombass/Pokemon-Hegemony/Release/version.txt"
  PATCH_LINK = $BETA ? "https://raw.githubusercontent.com/phantombass/Pokemon-Hegemony/master/patch.zip" : "https://raw.githubusercontent.com/phantombass/Pokemon-Hegemony/Release/patch.zip"
  def self.need_update?
    return false if $DEBUG
    vers = read_version
    return Downloader.toString(VERSION_LINK) != vers
  end

  def self.update
    msgwindow = Kernel.pbCreateMessageWindow
    Kernel.pbMessageDisplay(msgwindow, _INTL("Downloading patch...\\wtnp[0]"))
    new_vers = Downloader.toString(PATCH_LINK)
    pbDownloadToFile(PATCH_LINK,"patch.zip")
    self.applyPatch("patch.zip",'.')
  end

  def self.applyPatch(patch, directory)
    msgwindow = Kernel.pbCreateMessageWindow
    Kernel.pbMessageDisplay(msgwindow, _INTL("Applying patch...\\wtnp[0]"))
    begin
      gem 'zip'
      gem 'fileutils'
