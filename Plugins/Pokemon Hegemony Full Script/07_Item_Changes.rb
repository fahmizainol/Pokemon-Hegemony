...
def give_ability_orb
  ...
  item_list = [:INTIMIDATEORB,:LEVITATEORB,:ILLUMINATEORB,:FILTERORB,:DAZZLINGORB] if Restrictions.active?
...
def pbBattleConfusionBerry(battler,battle,item,forced,flavor,confuseMsg)
  ...
  fraction_to_heal = 2
  fraction_to_heal = 1.5 if battler.hasActiveAbility?(:RIPEN)
...
ItemHandlers::UseFromBag.add(:ESCAPEROPE,proc { |item|
  ...
  if $game_switches[Settings::GAUNTLET_SWITCH]
    pbMessage(_INTL("It can't be used when you are in a Gauntlet."))
    next 0
  end
...
BattleHandlers::TerrainStatBoostItem.add(:TOXICSEED,
  proc { |item,battler,battle|
    next false if battle.field.field_effects != :Poison
...
ItemHandlers::UseOnPokemon.add(:RARECANDY,proc { |item,pkmn,scene|
  if pkmn.level>=GameData::GrowthRate.max_level || pkmn.shadowPokemon? || (pkmn.level>=LEVEL_CAP[$game_system.level_cap] && $game_switches[Settings::LEVEL_CAP_SWITCH] == true && $game_switches[LvlCap::Expert] == false) || (pkmn.fainted? && $game_switches[73])
    scene.pbDisplay(_INTL("It won't have any effect."))
    next false
  end
...
BattleHandlers::ItemOnSwitchIn.add(:DURALUDONITE,
  proc { |ability, battler, battle|
    next if !battler.isSpecies?(:DURALUDON)
    battle.pbDisplay(_INTL("{1} gathers strength from its {2}!",
       battler.pbThis,battler.itemName))
    battler.pbRaiseStatStageByCause(:DEFENSE,1,battler,battler.itemName)
    battler.pbRaiseStatStageByCause(:SPECIAL_ATTACK,1,battler,battler.itemName)
    battler.pbRaiseStatStageByCause(:SPECIAL_DEFENSE,1,battler,battler.itemName)
  }
)