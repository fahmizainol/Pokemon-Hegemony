class PokeBattle_Battle
  def pbGetBestTypeJudgment(user, target, move = nil, check_type = nil)
    ...
    move = PokeBattle_Move.from_pokemon_move(self, PokeBattle_Move.new(:JUDGMENT)) if !move
...
class PokeBattle_Scene
  def pbArceusTransform(index, type = :NORMAL)
    @animations.push(PokeBattle_Animation::ArceusTransform.new(@sprites, @viewport, index, type))
...
#-------------------------------------------------------------------------------
class PokeBattle_Animation::ArceusTransform < PokeBattle_Animation
