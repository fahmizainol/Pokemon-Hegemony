...
module LvlCap
  ...
  def self.start_battle(type)
    ...
    when :Item
      $game_switches[LvlCap::Item] = true
    end
  end

  def self.end_battle
    type = [LvlCap::Gym,LvlCap::Boss,LvlCap::Rival,LvlCap::LvlTrainer,LvlCap::Item]
    for i in type
...
Events.onTrainerPartyLoad+=proc {| sender, trainer |
   if trainer # Trainer data should exist to be loaded, but may not exist somehow
     ...
     if $game_switches && $Trainer && $game_switches[LvlCap::Item]
      party.each do |mon|
        mon.level = LvlCap.cap - 1
        mon.calc_stats
      end
    end
   end
